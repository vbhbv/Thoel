...
async def _reject_callback_if_banned(query) -> bool:
    """فحص حظر موحّد لمعالِجات الأزرار في هذه الوحدة. ضروري لأن هذه
    المعالِجات تُسجَّل الآن قبل menu_callback العام في bot.py (الذي كان
    يقوم بهذا الفحص سابقًا لكن لم يعد يُستدعى أولًا لهذه الأزرار تحديدًا)."""
...
async def queue_image_processing(update: Update, context: ContextTypes.DEFAULT_TYPE, file_id: str, file_unique_id: str, filename: str):
    ...
    album = context.chat_data.setdefault("image_album", [])

    if len(album) >= MAX_IMAGES_PER_BATCH:
        await update.message.reply_text(
            f"⚠️ وصلت للحد الأقصى ({MAX_IMAGES_PER_BATCH} صورة). أرسل الآن اختيار الصيغة من الرسالة السابقة، "
            "أو انتظر قليلًا ثم ابدأ ألبومًا جديدًا."
        )
        return

    album.append({"file_id": file_id, "file_unique_id": file_unique_id, "filename": filename})
...
async def handle_image_conversion(update: Update, context: ContextTypes.DEFAULT_TYPE, target_format: str = None):
    """
    ⚠️ إصلاح جوهري: كانت target_format معاملًا إجباريًا بلا قيمة افتراضية،
    بينما هذه الدالة مسجَّلة كمعالِج CallbackQueryHandler مباشر (يستدعيها
    PTB بمعاملين فقط: update, context دائمًا). هذا كان يُسقط المعالِج
    بخطأ TypeError في كل ضغطة زر بلا استثناء، فلا يعمل دمج الصور إطلاقًا.
    الآن نستخرج الصيغة من بيانات الزر نفسها (query.data) عند عدم تمريرها.
    """
    ...
    if not album:
        await query.answer()
        await query.edit_message_text("⚠️ لم يتم العثور على الألبوم المطلوب. أرسل الصور من جديد.")
        return
...
async def handle_pdf_target_conversion(update: Update, context: ContextTypes.DEFAULT_TYPE, target_format: str = None):
    """⚠️ نفس إصلاح handle_image_conversion: target_format أصبح اختياريًا
    ويُستخرج من query.data عند الاستدعاء المباشر عبر CallbackQueryHandler."""
    query = update.callback_query
    if await _reject_callback_if_banned(query):
        return
    ...
    if not pending:
        await query.answer()
        await query.edit_message_text("⚠️ لم يتم العثور على ملف PDF معلّق. أرسل الملف من جديد.")
        return
...
async def process_pdf_encryption(update: Update, context: ContextTypes.DEFAULT_TYPE, password: str):
    ...
    if not password:
        await update.message.reply_text("⚠️ كلمة السر لا يمكن أن تكون فارغة. أرسل كلمة سر صالحة.")
        return
