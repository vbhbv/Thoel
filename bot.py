...
# متغير عالمي لتخزين تجمع اتصالات قاعدة البيانات لضمان كفاءة الإرسال ومنع التسريب
DB_POOL = None

# ----------------------------------------------------------------------
# منطق معالجة وقاعدة بيانات PostgreSQL (مُحدث لإضافة إعدادات الاشتراك وتجمع الاتصالات)
# ----------------------------------------------------------------------
...
async def init_db():
    ...
    try:
        # إنشاء تجمع اتصالات (Connection Pool) بدلاً من فتح وإغلاق الاتصال في كل حركة
        DB_POOL = await asyncpg.create_pool(url, min_size=2, max_size=10)
        async with DB_POOL.acquire() as conn:
            ...
            """)
        logger.info("✅ تم الاتصال بقاعدة البيانات PostgreSQL عبر الـ Pool وتهيئة الجداول بنجاح.")
...
async def run_cmd(*args: str) -> tuple[int, str, str]:
    # حماية معالجة العمليات الفرعية وتمرير المعاملات بشكل معزول وآمن بالكامل من الـ Shell Injection
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # فحص الحظر أولاً لحماية موارد البوت
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pdf_state == "WAITING_MERGE_FILES" and text.lower() in ["دمج", "merge"]:
    ...
    if state == "WAITING_MERGE_AUDIO" and text.lower() in ["دمج", "merge"]:
    ...
    # معالجة تصحيح الكلمات الإملائية لحالة تجميع الـ Metadata لضمان دقة التنقل
    if state == "WAITING_TITLE":
        context.user_data["meta_title"] = text
        context.user_data["audio_state"] = "WAITING_ARTIST"
        ...
        context.user_data["audio_state"] = "WAITING_ART"
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("audio_state") == "WAITING_ART":
