...
# ⚠️ ملاحظة مهمة: لا تستورد files_handler وaudio_handler هنا في الأعلى.
# كلا الملفين يحتاجان "from bot import DOWNLOADS_DIR, ..." وغيرها من
# الثوابت/الدوال المعرّفة لاحقًا في هذا الملف. لو استُوردا هنا قبل تعريف
# تلك الأسماء، يقع Python في استيراد دائري (Circular Import) لأن bot.py
# ما زال في منتصف التنفيذ (partially initialized module) عند تلك اللحظة.
# الاستيراد الصحيح موجود بعد آخر دالة يعتمدان عليها (finalize_and_send_audio)
# في أسفل هذا الملف — ابحث عن "استيراد وحدات المعالجة الخارجية" هناك.
...
# ----------------------------------------------------------------------
# استيراد وحدات المعالجة الخارجية (يجب أن يكون هنا وليس أعلى الملف)
# ----------------------------------------------------------------------
# كل الأسماء التي تستوردها files_handler.py وaudio_handler.py من bot.py
# (الثوابت، والدوال المساعدة، وأي أسماء أخرى معرَّفة أعلاه) مُعرَّفة الآن
# بالكامل بحلول هذه النقطة، لذا الاستيراد هنا آمن. لا تنقل هذا الاستيراد
# إلى أعلى الملف: كلا الوحدتين تنفّذان "from bot import ..." عند تحميلهما،
# ولو استُوردتا قبل اكتمال تعريف الأسماء التي تحتاجانها هنا في bot.py، يقع
# Python في استيراد دائري (Circular Import) لأن bot.py حينها لا يزال module
# غير مكتمل التهيئة (partially initialized).
try:
    import files_handler
except ModuleNotFoundError as e:
    # نلتقط تحديدًا "الملف غير موجود على الإطلاق" فقط (تدهور آمن ومتوقع).
    # لا نلتقط ImportError العام هنا عمدًا: خطأ الاستيراد الدائري (عندما
    # يكون الملف موجودًا لكن تسلسل الاستيراد خاطئ) يجب أن يظهر بوضوح
    # ويوقف التشغيل، لا أن يُخفى بصمت ويترك نصف ميزات البوت معطّلة دون تفسير.
    files_handler = None
    logger.critical(f"⚠️ ملف files_handler.py غير موجود: {e}")

try:
    import audio_handler
except ModuleNotFoundError as e:
    audio_handler = None
    logger.critical(f"⚠️ ملف audio_handler.py غير موجود: {e}")
